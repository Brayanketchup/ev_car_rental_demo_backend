...
app.post('/api/login', (req, res) => {
  ...
  if (!validateLength(identifier, 3, 100)) { 
...
app.post('/api/bookings', (req, res) => {
  ...
  const bookingStatus = status || 'on coming'; 
...
`;
  ...
  db.query(query, [carID], (err, results) => {
    ...
    if (results.length === 0) {
      ...
      // return res.status(200).json({ message: 'No reviews found for the specified car.' });
    }
...
//     return res
//       .status(400)
//       .json({ message: 'Identifier must be between 3 and 100 characters' });
//   }
//   if (!validateLength(password, 8, 100)) {
//     return res
//       .status(400)
...
//       message:
//         'Please provide all mandatory fields (manufacturer, model, year, seats, doors, price).',
//     });
//   }
//   if (!image) {
//     return res.status(400).json({ message: 'Image file is required.' });
...
//     'INSERT INTO Cars (manufacture, model, year, seats, doors, color, mileage, drive_type, price, description, image, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
//   db.query(
//     insertCarQuery,
//     [
//       manufacturer,
...
//         return res
//           .status(500)
//           .json({ message: 'Database error while adding new car', error: err });
//       }
//       res
//         .status(201)
//         .json({ message: 'New car added successfully', carId: result.insertId });
//     }
//   );
// });
...
//       return res
//         .status(500)
//         .json({ message: 'Database error while retrieving cars', error: err });
//     }
//     res.json(results);
//   });
// });
...
//       return res
//         .status(500)
//         .json({ message: 'Database error while retrieving car details', error: err });
//     }
//     if (results.length === 0) {
//       return res.status(404).json({ message: 'Car not found' });
//     }
...
// // Create a booking
// app.post('/api/bookings', (req, res) => {
//   const { carID, userId, tripStart, tripEnd, pickupReturnLocation, status } =
//     req.body;
...
//   db.query(
//     query,
//     [carID, userId, tripStart, tripEnd, pickupReturnLocation, bookingStatus],
//     (err, results) => {
//       if (err) {
//         console.error('Failed to create booking:', err);
//         return res
//           .status(500)
//           .json({ message: 'Database error while creating booking', error: err });
//       }
//       res
//         .status(201)
//         .json({ message: 'Booking created successfully', bookingID: results.insertId });
//     }
//   );
// });

// // Cancel booking (by ID in params)
// app.patch('/api/bookings/:bookingID/cancel', (req, res) => {
//   const { bookingID } = req.params;
//   if (!bookingID) {
//     return res.status(400).json({ message: 'Booking ID is required' });
...
//       return res
//         .status(500)
//         .json({ message: 'Database error while canceling booking', error: err });
//     }
//     if (results.affectedRows === 0) {
//       return res
//         .status(404)
...
//     res
//       .status(201)
//       .json({ message: 'Ticket created successfully.', ticket_id: results.insertId });
//   });
// });
...
//     return res
//       .status(400)
//       .json({ message: 'Booking ID, start date, and end date are required' });
//   }
...
//       return res
//         .status(500)
//         .json({ message: 'Database error while updating booking dates', error: err });
//     }
//     if (results.affectedRows === 0) {
//       return res.status(404).json({ message: 'Booking not found' });
//     }
...
//     return res
//       .status(400)
//       .json({ message: 'All fields (user_id, car_id, description, score) are required' });
//   }
//   if (score < 1 || score > 5) {
//     return res.status(400).json({ message: 'Score must be between 1 and 5' });
//   }
...
//       return res
//         .status(500)
//         .json({ message: 'Database error while creating review', error: err });
//     }
//     res.status(201).json({ message: 'Review created successfully', reviewID: results.insertId });
//   });
// });

// // Set a car to maintenance
// app.post('/api/car/setmaintenance/:carID', (req, res) => {
//   const { carID } = req.params;
//   if (!carID) {
//     return res.status(400).json({ message: 'Car ID is required' });
...
//       return res
//         .status(500)
//         .json({ message: 'Database error while updating car details', error: err });
//     }
//     if (results.affectedRows === 0) {
//       return res.status(404).json({ message: 'Car not found' });
//     }
...
// // Set a car to available
// app.post('/api/car/setavailable/:carID', (req, res) => {
//   const { carID } = req.params;
//   if (!carID) {
//     return res.status(400).json({ message: 'Car ID is required' });
//   }
...
//       return res
//         .status(500)
//         .json({ message: 'Database error while retrieving tickets', error: err });
//     }
//     res.json(results);
//   });
// });
